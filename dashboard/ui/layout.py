...
Builds the ``dash.Dash`` instance and its ``app.layout``. Kept free of
callback logic (see ``dashboard.callbacks``) so the visual structure can
                    ...
                    dmc.Stack([dmc.Paper(
                        dmc.Stack([
                            ...
                            dmc.Stack([
                                ...
                                dmc.MultiSelect(id="ticker-select", data=[{'label': t, 'value': t} for t in Config.TARGET_TICKERS], value=list(Config.TARGET_TICKERS)),
                                ], gap=2),
                            dmc.Stack([
                                dmc.Text("Period", fw=600, size="sm"),
                                dmc.DatePickerInput(id="date-range", type="range", minDate=min_date, maxDate=max_date, value=[min_date, max_date]),
                                dmc.Group([
